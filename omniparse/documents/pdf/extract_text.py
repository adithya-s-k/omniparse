...
from omniparse.documents.pdf.utils import font_flags_decomposer
from omniparse.documents.settings import settings
from omniparse.documents.schema.block import Span, Line, Block
...
def get_text_blocks(doc, fname, max_pages: Optional[int] = None, start_page: Optional[int] = None) -> (List[Page], Dict):
    ...
    omniparse_blocks = [pdftext_format_to_blocks(page, pnum) for pnum, page in enumerate(char_blocks)]

    return omniparse_blocks, toc
